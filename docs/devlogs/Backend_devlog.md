...
| **CUDA API 오남용 및 추론 병목 (VRAM Sync Bottleneck)** | 과거 문서상으로는 조치된 것으로 기록되어 있었으나, 실제 코드(`tracker.py`)의 정상 추론 루프 끝단에 `torch.cuda.empty_cache()`가 레거시로 남아 PyTorch의 메모리 재사용을 붕괴시키고 매 청크마다 GPU 스레드 강제 동기화(병목)를 유발함. | 정상 로직에서 VRAM 수동 초기화를 완벽히 소거하고, OOM이 발생한 `except RuntimeError` 블록(동적 백오프 상황)에서만 제한적으로 메모리를 회수하도록 코드를 문서와 100% 일치시켜 추론 속도 정상화. |
