...
---
## Final: 파이프라인 통합 후 음원 분리 아키텍처 최종 번복 (2-Stem -> 4-Stem)
* **개요:** 
    * Exp 007에서 시스템 효율을 위해 `htdemucs_two_stems`를 채택했으나, 실제 E2E 파이프라인 연동 후 한계가 발견되어 기존 결정을 번복함.
* **결과 분석 및 번복 사유:**
    * **VRAM 연산 착시 및 킥 드럼 간섭:** 2-Stem 옵션이 실제 VRAM 관리 측면에서 기대만큼의 이점이 없었음. 또한 2-Stem은 타 악기가 하나로 묶여 처리되다 보니, 베이스와 주파수 대역이 겹치는 킥 드럼의 간섭(Bleeding)을 완벽히 걸러내지 못하는 한계가 드러남.
* **최종 의사결정:**
    * **`htdemucs` (기본 4-Stem) 모델로 최종 복귀.**
    * 4-Stem 모델이 드럼을 별도 트랙으로 완벽히 분리하므로 킥 드럼 간섭 차단에 훨씬 유리함. 분리 후 CPU 연산을 통해 나머지 트랙(Drums + Vocals + Other)을 합산하여 MR을 만드는 방식으로 최종 아키텍처를 확정함.
